...
describe('InputView 테스트', () => {
  ...
  test.each(invalidDate)(
    'date가 유효하지 않을때 예외가 발생하는지 확인한다.',
    async (date) => {
      mockQuestions(date);
  ...
  test.each(validOrder)(
    'order가 유효할 때 값을 반환하는지 확인한다.',
    async (order) => {
      mockQuestions(order);
      expect(await InputView.readOrder()).toEqual(
        order.split(REGEXS.commaAndOptionalSpace)
      );
    }
  );
  ...
  test.each(invalidOrder)(
    'order가 유효하지 않을때 예외가 발생하는지 확인한다.',
    async (order) => {
      mockQuestions(order);
